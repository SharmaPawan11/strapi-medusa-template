...
module.exports = createCoreService(
  ...
  ({ strapi }) => ({
        ...
        if (data && data.length) {
          for (let shipping_profile of data) {
            ...
            const found = await strapi.db
              .query("api::shipping-profile.shipping-profile")
              .findOne({ where: { medusa_id: shipping_profile.medusa_id } });
            if (found) {
              continue;
            }

            const shippingOptionStrapiId =
              await createShippingProfileAfterDelegation(shipping_profile);
          }
        }
    ...
    async handleManyToOneRelation(shippingProfile, caller) {
      ...
      const found = await strapi.db
        .query("api::shipping-profile.shipping-profile")
        .findOne({ where: { medusa_id: shippingProfile.medusa_id } });
      if (found) {
        return found.id;
      }

      const shippingProfileStrapiId =
        await createShippingProfileAfterDelegation(shippingProfile);
